...
Data profiling — statistics, quality, PII detection, null patterns.
...
class MetadataAgent(BaseAgent):
    ...
    def process(self, ctx: AgentContext, result: AgentResult) -> dict:
        ...
        if not cfg or not cfg.get("host"):
            result.log("No connector config — skipping metadata profiling", "warn")
        ...
        if not tables:
            result.log("No tables to profile — skipping", "warn")
            ctx.metadata_result = {"profile": "", "pii_columns": [], "quality_issues": []}
            return ctx.metadata_result
        ...
        try:
            ...
            result.log(f"✓ Profile built ({len(profile_text)} chars)")
        except Exception as e:
            result.log(f"Profiling failed: {e} — skipping", "warn")
            profile_text = ""
        ...
        if pii_columns:
            result.log(f"⚠ PII detected: {pii_columns}", "warn")
        else:
