...
const glow = (color = C.cyan, strength = 4) => 'none';
...
function NeonLineChart({ data, labelKey, valueKey }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ height: 130 }}>
      ...
      <path d={path} fill="none" stroke={C.pink} strokeWidth="1.5"
        style={{}} />
      {vals.map((v, i) => (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {slices.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
            ...
            <span style={{ marginLeft: "auto", paddingLeft: 8, color: s.color, }}>{s.pct}%</span>
          </div>
        ))}
      </div>
    </div>
...
export default function AIBridgeNeonBI() {
  ...
  const [pipeline, setPipeline] = useState("67b29244-bdf1-4b0d-8b97-46eae637be45");
  ...
  async function ask() {
    ...
    try {
      // Use /chat endpoint — knows real warehouse schema
      const chatRes = await fetch("http://localhost:8888/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${localStorage.getItem('aibridge_token') || ''}` },
        body: JSON.stringify({
          message: q,
      ...
      const sr = chatData.sql_result;
      const hasData = sr && sr.columns && sr.rows && sr.rows.length > 0;
      const cleanRows = hasData
        ? sr.rows.map(row => row.map(v => {
      ...
      setMessages(m => [...m, {
        role: "assistant",
        text: chatData.response || "Query complete.",
        sql: sr?.sql || null,
        columns: hasData ? sr.columns : [],
        rows: cleanRows,
        question: q,
      }]);
    } catch (err) {
    ...
    <div style={{ ...base, minHeight: 600, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          letterSpacing: "0.2em", color: C.cyan, }}>◈ AIBRIDGE BI</div>
          ...
          <div key={i} className={msg.role === "user" ? "msg-user" : "msg-ai"}>
            ...
            {msg.role === "error" && (
              <div style={{
                ...
                fontSize: 11, color: C.red, }}>{msg.text}</div>
            )}
